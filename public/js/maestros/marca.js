...
$(document).ready(function(){
	$('#table-marca').DataTable({ responsive: true,
		...
		autoWidth: false,
		ajax:{
			url : base_url + 'index.php/maestros/marca/datatable_marca/',
			type: "POST",
			data: { dataString: "" },
			beforeSend: function(){
...
function editar(id){
	var url = base_url + "index.php/maestros/marca/getMarca";
	$.ajax({
		type: 'POST',
		url: url,
		dataType: 'json',
...
function registrar_marca(){
	Swal.fire({
		...
		if (result.value){
			...
			if (validacion == true){
				var url = base_url + "index.php/maestros/marca/guardar_registro";
...
function deshabilitar(marca){
	Swal.fire({
		...
		if (result.value){
			var url = base_url + "index.php/maestros/marca/deshabilitar_marca";
			$.ajax({
				type: 'POST',
				url: url,
				dataType: 'json',
